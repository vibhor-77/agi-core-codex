...
LEGACY_PROFILES = ("baseline-core", "arc-accuracy", "arc-theory", "arc-transfer", "arc-bootstrap")


def _build_legacy_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="agi-core-codex legacy")
    ...
    make_splits_parser.add_argument("--benchmark", choices=("arc-agi-1", "arc-agi-2"), required=True)
    ...
    arc_analyze_subparsers = arc_analyze_parser.add_subparsers(dest="arc_analyze_command", required=True)
    ...
    for profile in LEGACY_PROFILES:
        ...
        for mode in ("tune", "score"):
            ...
            mode_parser.add_argument("--benchmark", choices=("arc-agi-1", "arc-agi-2"))
            mode_parser.add_argument("--dataset-split", choices=("training", "evaluation"), default="training")
            mode_parser.add_argument("--include-strategy", action="append", default=[])
            mode_parser.add_argument("--exclude-strategy", action="append", default=[])
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.profile == "arc-data":
        ...
        if args.arc_data_command == "make-splits":
            dataset_dir = args.dataset_dir or discover_arc_dataset(benchmark=args.benchmark, split="training")
            if dataset_dir is None:
                parser.error("could not auto-discover ARC training data; pass --dataset-dir explicitly")
    ...
    if args.profile == "arc-analyze":
        if args.arc_analyze_command == "scan-primitives":
            ...
            print(format_scan_results(results, output_format=args.format, top=args.top))
            return 0
        if args.arc_analyze_command == "classify-failures":
            report = classify_failures(manifest_path=args.manifest_path, dataset_dir=args.dataset_dir)
        ...
        if args.arc_analyze_command == "cluster-failures":
            report = cluster_transfer_failures(manifest_path=args.manifest_path, dataset_dir=args.dataset_dir)
